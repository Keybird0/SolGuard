...
### Added — v0.9 · A3 Deep-Dive + Gate1/4 hardening + Claude Code multi-runtime (2026-04-26)
- **A3 Deep-Dive Agent (v0.9, pure playbook)** — `references/l3-agents-playbook.md` §3 adds a third L3 agent that runs after A1+A2 merge to catch four blind-spot classes A1/A2 cannot see from a single-handler view: `sibling-drift` (two handlers share an Accounts struct but only one has the canonical guard) · `cross-cpi-taint` (caller-controlled bytes flow through internal helpers to `invoke*` without revalidation) · `callee-arith` (handler-entry math looks safe but a depth-≤2 callee multiplies/adds without `checked_*`) · `authority-drop` (handler validates `Signer` but inner fn no longer carries the constraint). Pure playbook — **zero new Python tool, zero `skill.yaml` change, zero `solguard-server` change** (the `Candidate.source` literal `"A3"` was pre-allocated in v0.8 at `ai/agents/types.py:29`). Risky-handler selection: top-N=5 from A1+A2 merge by `(severity desc, agents desc)`; A3 auto-skips with **0 LLM cost** when A1+A2 merge yields 0 candidates AND no instruction matches the mut+invoke heuristic. L3 token budget: ~13k (A3 skip) / ~21k (A3 fires) — both well under `SOLANA_AUDIT_BUDGET=50_000`. SKILL.md Step 5.L3 table + README/README.zh-CN L3 stage tables updated to list A3.
- **Gate1 / Gate4 hardening — fix systematic FN paths surfaced by v0.9 verification** (commit `c66db45`):
  - `ai/judge/kill_signal.py`: when a candidate's line cannot be located inside an enclosing `#[derive(Accounts)]` struct or `pub fn` body, `_find_owner_struct` / `_find_owner_function` now return `None` and `apply()` records the signal under `signals_skipped_no_scope[]` instead of falling back to whole-source regex (which previously caused over-KILL — any kill-signal that happened to match elsewhere in the file would falsely KILL the candidate). Explicit `scope="file"` (KB author opt-in) is unchanged. Under-KILL is the safe direction (the candidate proceeds into Gate2/3 LLM judgment).
  - `ai/judge/seven_q_gate.py`: `_rule_in_scope` returns `(True, provisional=True)` for `rule_id=None` (A1 Explorer "truly novel" findings per `l3-agents-playbook.md §1.1`) instead of `False`. The `q3_provisional` flag is recorded in the answers ledger so Gate2/3 evidence — not Q3 alone — decides the outcome. Hard-fail (KILL) still fires for non-null rule_ids absent from the KB or with explicit external-analyzer prefixes (`semgrep:` / `external:` / etc.).
  - `tests/test_ai_judge.py` adds 2 regression tests: `test_gate1_skips_signal_when_scope_unresolvable` + `test_gate4_provisional_pass_when_rule_id_is_none`.
- **Claude Code dispatcher** (commit `0fc9790`) — `scripts/skill_tool.py` (~165 lines) exposes all 9 thin tools (`parse`, `scan`, `semgrep`, `kill_signal`, `cq_verdict`, `attack_classify`, `seven_q`, `judge_lite`, `report`) through one stdin/stdout JSON CLI without modifying any Tool class. Lets the SKILL run on Claude Code (in addition to OpenHarness) via symlink installation: `ln -s SolGuard/skill/solana-security-audit-skill ~/.claude/skills/solana-security-audit-skill`. SKILL.md gains a "Claude Code Invocation Pattern" section (~98 lines) documenting the symlink install, six-step Bash sequence with `jq`, and OpenHarness-vs-Claude-Code differences. `tests/test_skill_tool.py` adds 10 subprocess-driven dispatcher tests covering tool listing, invalid-payload rejection, bad-kwargs trap, and end-to-end round-trips for 7 of the 9 tools.
- **Cashio infinite-mint PoC fixture** (commit `30bd77d`) — `test-fixtures/known-incidents/cashio_poc.rs` (~94 lines) reproduces the 2022-03 Cashio root cause (~$52M loss) in single-file form: raw `AccountInfo` for `collateral_tokens` + manual `try_from_slice` → attacker supplies a synthetic account with `cash_mint` field pointing at the real CASH mint and `weight=u64::MAX`, then mints unbounded supply. Used by the v0.9 verification to confirm SolGuard catches the literal historical incident.
- **End-to-end verification on 5 real targets** (`outputs/verifi/`, gitignored locally) — `sealevel-arbitrary-cpi` (rw04, 1 Critical) · `sealevel-multi-vuln-amm` (rw11, 1 Critical + 1 High [sibling-drift] from A3) · `clean-escrow` (rw08, 0 finding control) · `cashio-poc` (1 Critical + 1 High, literal $52M root cause) · `chillhouse-token` (real SPL Token `GkyPYa7Nn...` via `bytecode_only` path). Aggregate Precision 1.00 / Recall 0.83 / F1 0.91 on this 5-target slice (Claude Opus 4.7 as Agent — not directly comparable to phase6-round2 `gpt-5.4` baseline). Surfaced one real KB authoring gap: `integer_overflow` is missing from `knowledge/solana_bug_patterns.json`, causing Gate4 Q3 to KILL valid integer-overflow candidates on 2 of the 5 targets — tracked as VF-001.
- **Documentation refresh** (this turn) — `docs/README.md` updated: all 7 Phase-7-planned documents marked ✅ DONE with correct paths (5 already existed pre-this-session); demo path corrected from `demo-script.md` to `script.md`; API.md is intentionally omitted in favor of the `openapi.yaml` source-of-truth + Swagger UI at `/docs`. README + README.zh-CN Roadmap section updated: M2 added as ✅ done; six "Forward-looking themes" rewritten against the v0.9 baseline (VF-001 promoted to top, deprecated themes removed). Top-level READMEs Features section gains "Multi-runtime SKILL" bullet.
